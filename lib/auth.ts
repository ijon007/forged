...
const updateUserSubscription = async (customerId: string, updates: any) => {
  try {
    ...
    console.log(
      `Updated user subscription for customer ${customerId}:`,
      updates
    );
    return result;
  } catch (error) {
    console.error(
      `Failed to update user subscription for customer ${customerId}:`,
      error
    );
    throw error;
  }
    ...
    polar({
      ...
      use: [
        checkout({
          products: [
            {
              productId: process.env.NEXT_PUBLIC_MONTHLY_ID!,
              slug: "forged",
            },
            {
              productId: process.env.NEXT_PUBLIC_YEARLY_ID!,
              slug: "forged-yearly",
            },
          ],
