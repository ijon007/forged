...
/* Types */
import { ContentType, CONTENT_TYPES, CourseContent } from '@/db/schemas/course-schema'

/* Utils */
import { generateSlug } from '@/utils/slug'
import { autoSelectImageFromTitle } from '@/lib/unsplash-api'
import { getBlogPrompt, getBlogSystemPrompt, getListiclePrompt, getListicleSystemPrompt, getCoursePrompt, getCourseSystemPrompt } from '@/utils/prompts'
import { courseSchema, listicleSchema, blogSchema } from '@/utils/zod-schemas'
...
async function parsePDFToText(file: File): Promise<{ success: boolean; text?: string; error?: string }> {
    try {
        if (!file) {
...
export async function generateCourseFromPDF(formData: FormData): Promise<CourseGenerationResult> {
    try {
        ...
        const systemPrompt = 
            contentType === CONTENT_TYPES.LISTICLE ? getListicleSystemPrompt(title, description, price) : 
            contentType === CONTENT_TYPES.COURSE ? getCourseSystemPrompt(title, description, price) :
            getBlogSystemPrompt(title, description, price)

        const promptText = 
            contentType === CONTENT_TYPES.LISTICLE ? getListiclePrompt(extractedText) : 
            contentType === CONTENT_TYPES.COURSE ? getCoursePrompt(extractedText) :
            getBlogPrompt(extractedText)

        const result = contentType === CONTENT_TYPES.COURSE ? 
        ...
        const contentForStore = contentType === CONTENT_TYPES.COURSE ? 
