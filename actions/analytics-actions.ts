...
async function getUserPolarData() {
  ...
  if (
    userData[0].polarTokenExpiresAt &&
    new Date() > userData[0].polarTokenExpiresAt
  ) {
    try {
      ...
      if (
        !(
          ...
          updatedUserData[0].polarAccessToken &&
          updatedUserData[0].polarOrganizationId
        )
      ) {
        return null; // Return null instead of throwing error
      }
...
async function fetchPolarOrders(
  accessToken: string,
  ...
  if (!response.ok) {
    ...
    if (response.status === 403) {
      ...
      if (errorData.error === "insufficient_scope") {
        throw new Error(
          "Insufficient permissions. Please disconnect and reconnect your Polar account to grant the orders:read permission."
        );
      }
    }
...
export async function getAnalyticsStats(): Promise<AnalyticsStats> {
  try {
    ...
    if (!userData) {
      // Return default values when no Polar account is connected
      return {
        totalRevenue: 0,
        monthlyRevenue: 0,
        revenueGrowth: 0,
    ...
    console.log(
      "Fetching analytics stats for organization:",
      userData.polarOrganizationId
    );

    const orders = await fetchPolarOrders(
      userData.polarAccessToken,
      userData.polarOrganizationId
    );
...
export async function getMonthlyChartData(): Promise<MonthlyData[]> {
  try {
    ...
    if (!userData) {
      // Return default data for last 12 months when no Polar account is connected
      const months = [];
      const now = new Date();
...
export async function getRecentCustomers(): Promise<RecentCustomer[]> {
  try {
    ...
    if (!userData) {
      // Return empty array when no Polar account is connected
      return [];
    }
    ...
    return completedOrders.slice(0, 10).map((order: any) => {
      ...
      const courseName = order.product_id
        ? productToCourseMap.get(order.product_id) ||
          order.product?.name ||
          "Unknown Course"
        : "Unknown Course";
