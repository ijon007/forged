...
export async function getPolarConnectionStatus() {
  try {
    ...
    if (
      isConnected &&
      !isTokenExpired &&
      userPolarData.polarAccessToken &&
      userPolarData.polarOrganizationId
    ) {
      try {
        const orgResponse = await fetch(
          `https://api.polar.sh/v1/organizations/${userPolarData.polarOrganizationId}`,
          {
            headers: {
  ...
  try {
    ...
    if (!userPolarData.polarOrganizationId) {
      return {
        success: false,
        error:
          "Polar organization not found. Please reconnect your Polar account.",
      };
    }

    // Check if token is expired
    if (
      userPolarData.polarTokenExpiresAt &&
      new Date() > userPolarData.polarTokenExpiresAt
    ) {
      try {
        ...
        return {
          success: false,
          error: "Polar token expired and could not be refreshed",
        };
      }
    }
    ...
    return {
      ...
      error:
        error instanceof Error
          ? error.message
          : "Failed to create Polar product",
    };
  }
}
  ...
  try {
    ...
    if (
      ...
      return {
        success: false,
        error:
          "Your Polar account needs to be reconnected with updated permissions. Please disconnect and reconnect your Polar account to enable checkout creation.",
      };
    }
...
export async function archivePolarProduct(productId: string) {
  try {
    ...
    const response = await fetch(
      `https://api.polar.sh/v1/products/${productId}`,
      {
        method: "PATCH",
        headers: {
